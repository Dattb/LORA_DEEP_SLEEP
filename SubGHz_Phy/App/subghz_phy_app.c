...
#include "../../Core/Src/max44009_sensor.h"
//#include "App"
...
unsigned int rd_task_cnt = 0;
unsigned char RF_one_time = 0;
 ...
 * @retval none
 */
 static void Rd_rf_process(void);
...
//	UTIL_TIMER_SetPeriod(&timerLed, 3000);

//	UTIL_TIMER_Start(&timerLed);
	
	UTIL_TIMER_Create(&timerLed1, 0xFFFFFFFFU, UTIL_TIMER_ONESHOT, timer_event, NULL);
	UTIL_TIMER_SetPeriod(&timerLed1, 1000);
...
#error user to provide its board code or to call his board driver functions
  ...
  UTIL_SEQ_RegTask((1 << CFG_SEQ_Task_PingPong_Process), UTIL_SEQ_RFU, PingPong_Process);
	UTIL_SEQ_RegTask((1 << CFG_SEQ_Task_RD_Test_RF), UTIL_SEQ_RFU, Rd_rf_process);
...
unsigned char wakeup_cnt = 0;

// RD_EDIT: su dung ham nay de tinh moc thoi gian
unsigned char led_db_cnt = 0;
void blink_led_db(unsigned char time){
	write_led(1);
	HAL_Delay(time);
	write_led(0);
}
...
static void OnledEvent(void *context)
{
	led_db_cnt++;
	if(led_db_cnt == 5){
		UTIL_TIMER_SetPeriod(&timerLed, 1000);
	}
	else if(led_db_cnt >5) led_db_cnt = 6;
	blink_led_db(5);
	UTIL_TIMER_Start(&timerLed);
}

unsigned char network_status_old = 0,network_status_new = 0;
static void timer_event(void *context)
{
	static unsigned int time_send = 0;
	rd_task_cnt++;
	network_status_new = network_status;
	if(network_status_old != network_status_new){
		network_status_old = network_status_new;
		rd_task_cnt = 0;
	}
	if(network_status == NOT_JOINT){
		if(rd_task_cnt >= 5 && rd_task_cnt <7){
			wakeup_flag = 1;
			RF_one_time = 1;
			UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_PingPong_Process), CFG_SEQ_Prio_0);
		}
		else if(rd_task_cnt >= 7) {
			Radio.Sleep();
			wakeup_flag = 0;
			rd_task_cnt = 0;
		}
	}
	else if (network_status == JOINED){
	 //wakeup_flag = 0;
		/*
		//RD_EDIT: 4 byte dem duoc 4,294,967,295 => thoi gian ngau nhien tu 1 => 60 la :
		 (float) dev_key_32/4294967295*60   => random_time = (4294967295/60) = 71582788 
		*/
		unsigned char time_random = dev_key_32;
		time_random = time_random/25;
		//if(rd_task_cnt < (10 + time_random)) Radio.Sleep();
		if(rd_task_cnt >= (10 + time_random) && rd_task_cnt <(13 + time_random)){
			if(rd_task_cnt == 10){
				 light_sensor_read_data();
			}
			UTIL_TIMER_SetPeriod(&timerLed1, 500);
			wakeup_flag = 1;
			RF_one_time = 1;
			UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_PingPong_Process), CFG_SEQ_Prio_0);
		}
		else if(rd_task_cnt >= (13 + time_random)) {
			UTIL_TIMER_SetPeriod(&timerLed1, 1000);
			Radio.Sleep();
			wakeup_flag = 0;
			rd_task_cnt = 0;
...
void UTIL_SEQ_Run_RF(unsigned char *counter){
...
int  op_gw_to_light_handle(unsigned char *par,rd_direction_t direction_handle){
	#if RD_LORA_MODEL_SEL == RD_LORA_MODEL_LIGHT_SENSOR
		unsigned char	opcode = par[1]<<8|par[0];
	#endif
	return 0;
}
